...
inline mesh* cg_load_mesh( const char* vert_binary_path, const char* index_binary_path )
{
	...
	// load vertex buffer
	mem_t v = cg_read_binary(vert_binary_path);//파일 읽어서 메모리 할당, return
...
// assume stb_image.h included with STB_IMAGE_IMPLEMENTATION
// - unless cg_load_image() is called, the compiler would't complain
extern "C" unsigned char* stbi_load(const char*,int*,int*,int*,int);
inline image* cg_load_image( const char* image_path, bool alpha=false, bool vflip=true )
{
	if(access(image_path,0)!=0){ printf("%s(): %s not exists.\n", __func__, image_path); return nullptr; }
	if(access(image_path,4)!=0){ printf("%s(): %s does not have read access.\n", __func__, image_path); return nullptr; }

	image* i = new image;
	unsigned char* i0 = stbi_load( image_path, &i->width, &i->height, &i->channels, alpha?4:3 );
	if(!i0||!i->width||!i->height||!i->channels) return nullptr;
	int stride0=i->width*i->channels, stride1=(stride0+3)&(~3);	// alignment at 4-byte boundary
	i->ptr = (unsigned char*) malloc( sizeof(unsigned char)*stride1*i->height );
	for( int y=0, h=i->height; y < h; y++ ) memcpy( i->ptr+(vflip?(h-1-y):y)*stride1, i0+y*stride0, stride0 ); // copy row with vertical flip
